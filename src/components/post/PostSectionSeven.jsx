...
const PostSectionSeven = ({ postData }) => {
  const videoStory = postData.filter(
    (post) => post.postFormat === "video" && post.story === true
  );
      ...
      <div className="container">
        ...
        <div className="row">
          <div className="col-lg-6">
            {videoStory.slice(0, 1).map((data, index) => (
              <PostVideoThree data={data} key={index} />
            ))}
          </div>
          <div className="col-lg-6">
            <div className="axil-content">
              {storyPost.slice(1, 5).map((data, index) => (
                <PostLayoutTwo data={data} postBgDark={true} key={index} />
              ))}
            </div>
          </div>
        </div>
